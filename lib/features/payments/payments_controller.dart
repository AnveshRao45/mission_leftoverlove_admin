...
class PaymentController {
  ...
  Future<PaymentSummary> fetchPayments(int restaurantId, String filter) async {
    final validFilters = ['today', 'week', 'month', 'year', 'total'];
    if (!validFilters.contains(filter)) throw Exception('Invalid filter type');
    return await _repository.getPayments(restaurantId, filter);
  }

  Future<Map<String, double>> fetchPaymentSummary(int restaurantId) async {
    return await _repository.getPaymentSummary(restaurantId);
  }
}